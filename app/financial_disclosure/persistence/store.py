...
class PersistenceStore:
    ...
    def record_verification_run(
        ...
        with self._lock:
            self._conn.execute(
                "INSERT INTO verification_run(run_id, filing_id, status, result)"
                " VALUES (?,?,?,?)",
                (run_id, filing_id, status, serialized),
            )
            self._conn.commit()
    ...
    def get_verification_run(
        ...
        with self._lock:
            row = self._conn.execute(
                "SELECT filing_id, status, result FROM verification_run WHERE run_id=?",
                (run_id,),
            ).fetchone()
        if row is None:
            return None
